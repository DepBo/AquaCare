...
# -------- DashboardPage.tsx --------
path_web = "c:/Users/Acer/Downloads/AquaCare/AquaCare_Web/src/pages/DashboardPage.tsx"
with open(path_web, "r", encoding="utf-8") as f:
    content = f.read()

# 1. Update SensorData interface
content = re.sub(
    r'interface SensorData \{[\s\S]*?\}',
    r'''interface SensorData {
  ph: SensorReading[]
  tds: SensorReading[]
  temp: SensorReading[]
...
# 2. Update initSensor
content = re.sub(
    r'function initSensor\(\): SensorData \{[\s\S]*?\}',
    r'''function initSensor(): SensorData {
  return {
...
content = re.sub(
    r'setSensorData\(\{\s*ph: generateHistory.*?\n.*?salinity: generateHistory.*?\n\s*\}\)',
    r'''setSensorData({
...
content = re.sub(
    r'return \{\s*ph: append.*?salinity: append.*?\n\s*\}',
    r'''return {
...
content = re.sub(
    r'if \(latest < cfg\.warn\[0\] \|\| latest > cfg\.warn\[1\]\) \{[\s\S]*?\}',
    r'''if (cfg.key === 'waterLevel') {
...
content = re.sub(
    r'<Sparkline data=\{hist\.slice\(-16\)\} color=\{cfg\.color\} height=\{48\} width=\{220\} />[\s\S]*?</span>\n\s*</div>',
    r'''{cfg.key === 'waterLevel' ? (
...
content = re.sub(
    r'<div style=\{\{ display: \'flex\', alignItems: \'flex-end\', justifyContent: \'space-between\', marginBottom: 24 \}\}>[\s\S]*?</svg>\n\s*</div>',
    r'''{selectedSensor === 'waterLevel' ? (
...
content = re.sub(
    r'<div style=\{\{ display: \'grid\', gridTemplateColumns: \'repeat\(4,1fr\)\', gap: 12 \}\}>[\s\S]*?</div>\n\s*</>',
    r'''{selectedSensor !== 'waterLevel' && (
...
content = re.sub(
    r'\[\s*\{\s*icon: \'💧\', title: \'pH bất thường\', desc: \'Sử dụng vôi để tăng pH hoặc thêm axit hữu cơ để giảm\.\' \},[\s\S]*?\]\.map\(t => \(',
    r'''[
...
content = re.sub(
    r'<div style=\{\{ fontSize: 32, fontWeight: 700, color: sc, marginBottom: 12, letterSpacing: \'-0\.02em\' \}\}>\n\s*\{val\}\{cfg\.unit\}\n\s*</div>',
    r'''<div style={{ fontSize: 32, fontWeight: 700, color: sc, marginBottom: 12, letterSpacing: '-0.02em' }}>
...
path_app = "c:/Users/Acer/Downloads/AquaCare/AquaCare_App/lib/screens/dashboard_screen.dart"
with open(path_app, "r", encoding="utf-8") as f:
    content = f.read()

# 1. Update sensorList
content = re.sub(
    r'final List<SensorData> sensorList = \[[\s\S]*?\];',
    r'''final List<SensorData> sensorList = [
  SensorData(
    name: 'pH',
    unit: '',
    value: 7.20,
...
# 2. Update alertList
content = re.sub(
    r'final List<AlertItem> alertList = \[[\s\S]*?\];',
    r'''final List<AlertItem> alertList = [
  AlertItem(
    title: 'pH ổn định',
...
content = content.replace("'6 hoạt động'", "'4 hoạt động'")
content = content.replace("'Tất cả 6 cảm biến trong ngưỡng an toàn'", "'Tất cả 4 cảm biến trong ngưỡng an toàn'")
