...
class SupabaseService {
  ...
  Future<List<Map<String, dynamic>>> getTanks(String userId) async {
    ...
    final response = await client.from('tanks').select('*').eq('user_id', userId);
  ...
  Stream<List<Map<String, dynamic>>> getTelemetryStream(String tankId, {int limit = 10}) async* {
    debugPrint('🔍 [DB QUERY]: Init realtime stream for tankId: $tankId');
    
    // Do stream() không hỗ trợ JOIN, ta phải lấy device_id trước
    final devices = await client.from('devices').select('id').eq('tank_id', tankId);
    if (devices.isEmpty) {
  ...
  Stream<List<Map<String, dynamic>>> getAlertsStream(String tankId, {int limit = 50}) async* {
  ...
  Future<List<Map<String, dynamic>>> getAlertsWithPagination(
    ...
    debugPrint('🔍 [DB QUERY]: Fetching alerts for tankId: $tankId, type: $type, offset: $offset');
    try {
      var query = client.from('alerts_history').select('*').eq('tank_id', int.parse(tankId));
  ...
  Future<Map<String, int>> getAlertDistribution(String tankId) async {
    debugPrint('🔍 [DB QUERY]: Fetching alert distribution for tankId: $tankId');
    try {
      final response = await client
          .from('alerts_history')
          .select('alert_type')
  ...
  Stream<Map<String, dynamic>?> getDeviceStream(String tankId) async* {
    ...
    final devicesRes = await client.from('devices').select('id').eq('tank_id', int.parse(tankId));
  ...
  Future<void> updateRelayState(String tankId, String relayType, bool newState) async {
    try {
      ...
      await client
          ...
          .eq('tank_id', int.parse(tankId));
      debugPrint('✅ [DB UPDATE]: Cập nhật $columnName thành $newState cho bể $tankId');
    } catch (e) {
  ...
  Future<void> updateDeviceSchedule(String tankId, String field, String? time) async {
    try {
      await client
          .from('devices')
          .update({field: time})
          .eq('tank_id', int.parse(tankId));
      debugPrint('✅ [DB UPDATE]: Cập nhật hẹn giờ $field thành $time cho bể $tankId');
    } catch (e) {
